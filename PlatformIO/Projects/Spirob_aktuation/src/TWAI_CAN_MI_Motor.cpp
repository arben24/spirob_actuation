...
static void CAN_Send_Frame(can_frame_t* frame)
{
    ...
    uint32_t combined_val = 0;   // must start at 0: only ever OR'd into below
...
/**
      * @brief          请求反馈帧(comm type 2)，不改变电机状态，可用作ping/主动刷新遥测
      *                 电机收到任意帧后才会回发状态帧，多电机共享总线时需定期调用本函数逐个刷新
      */
void MI_Motor_::Motor_Request_Feedback(void)
{
    can_frame_t motor_con_frame;
    motor_con_frame.mode = 2;           //模式2，反馈帧(与电机主动上报的状态帧类型相同)
    motor_con_frame.id = this->id;      //目标电机ID
    motor_con_frame.data = MASTER_ID;   //本机ID
    for (int i = 0; i < 8; i++)
    {
